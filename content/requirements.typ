...
== Overview
#TODO[
  The following parapgraph needs a refactor
]
...
== Existing System
Artemis (version 8.3.4) currently allows students to request automatically generated feedback for programming, modelling, and text exercises, provided the lecturer enables this feature. Each student may submit up to ten feedback requests per exercise. When a request is triggered, Artemis forwards the submission and exercise metadata—problem statement, example solution, rubric, and maximum points—to Athena. Athena selects an exercise-specific feedback module that relies on an LLM prompt template and chain-of-thought reasoning to produce formative feedback aligned with the rubric. The generated feedback is returned to Artemis, which displays it in two categories: referenced feedback (anchored to specific lines or elements of the submission) and unreferenced feedback.
...
#fr("FR 3", "Extract Competencies", "The system should identify the competencies required for each exercise by analysing the provided metadata using a recognised taxonomy such as Bloom's Taxonomy - when the lecturer does not provide them through the LMS.")
...
#fr("FR 7", "Utilise Engagement Data", "The system should incorporate engagement signals (i.e., forum posts, chat logs) extract the insights about where the student is struggling when assessing the competency status.")
...
#fr("FR 9", "Update Learner Profiles", "The system should refine the learner profile after changes in competency status and explicit preference settings. These changes can be triggered when a student submits a new submission, engages with the platform's chatbot, or when the student changes their feedback preferences.")
...
#fr("FR 12", "View Personalised Feedback", "The system should display the personalised feedback to the student in a way that makes it easy to understand what the feedback is about, and what the next steps are to improve.")
...
#fr("QA 2", "Preference Interface", "The feedback preference-setting workflow should be easy to understand and complete. It should be intuitive to use and should not take more than 1 minute for students to configure. Interface should adhere to Artemis design conventions.")
...
This section contains the system models for the requirements. First, the scenarios are presented for the proposed system, followed by the use case models to detail the scenarios. The dynamic model and the analysis object model give more details regarding the inner dynamics of the proposed system and how the objects are represented. Finally, the user interface mockups for the proposed system are presented. The diagrams in this section adhere to Unified Modeling Language (UML) to set a standard for modeling demonstrations.
...
#text(weight: "bold")[Preference Configuration and Iterative Adaptation.] Anna, a first-semester student, often gets distracted reading long paragraphs. Before solving the rate-limiter exercise, she visits the newly deployed Feedback Preferences page, where an interactive wizard previews different styles. She selects brief and formal as her feedback style. The system stores these choices in her learner profile.
...
Ben remains unsure why fixed-window counters can outperform token buckets. In the course forum, he posts a short question: "Why opt for a fixed window instead of a token bucket despite the risk of bursts?" The forum post is automatically incorporated into his learner profile.
...
=== Use Case Model
Under this subsection, we present the use case model for the proposed system, which is based on the scenarios presented in the previous section. It showcases the actions students can initiate - as use case models present the functionalities of a system that can be initiated by the users of the system @bruegge2009.
...
To provide a high-level abstraction of the problem domain and how we initially modelled it, we use the Analysis Object Model (AOM). AOM covers the application domain's crucial objects, attributes, methods, and relations that would guide the system's design throughout the implementation @bruegge2009. In this section, we present the AOM for the proposed system, as can be seen in @aom.
...
More detail, #text(weight: "bold")[Student Competency Status] maintains a running estimate of each learner's mastery for every required concept. #text(style: "italic")[Progress] classify the status of the students regarding the competency, while #text(style: "italic")[evidence] links to concrete evidence from students' submissions. The operation #text(style: "italic")[assessCompetencyStatus()] recalculates the mastery level whenever new feedback is requested; #text(style: "italic")[utiliseSubmissionHistory()] makes sure that the previous submissions are taken into account when the competency status is assessed. A solid association ties the class to #text(weight: "bold")[Competency], signalling that each status record corresponds to one instructional target extracted from the exercise specification.
...
#figure(caption: "Dynamic Model 2: Student Configures Feedback Preferences")[
  #image("../figures/dynamic_2.svg", width: 90% ,format: "svg")
]
...
#figure(caption: "Feedback Preferences Setup Mockup")[
  #image("../figures/feedback-component/mockup_1.png", width: 100%)
] <mockup1>
...
#figure(caption: "Feedback Component Mockup")[
  #image("../figures/feedback-component/mockup_2.png", width: 100%)
] <mockup2>
